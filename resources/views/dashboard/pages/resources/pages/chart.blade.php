                    ...
                    <select id="program_id" name="program_id"
                        ...
                        @foreach ($programs as $program)
                            <option value="{{ $program->id }}"
                                {{ request('program_id') == $program->id ? 'selected' : '' }}>{{ $program->name }}</option>
                        @endforeach
                    </select>
                </div>
                    ...
                    @else
                        @foreach ($agendas as $agenda)
                            <tr>
                                ...
                                <td rowspan="2" class="border border-gray-300 px-4 py-2 text-nowrap">
                                    {{ $agenda->title }}</td>
                                ...
                                @for ($i = 1; $i <= 30; $i++)
                                    @php
                                        $date = date(
                                            'Y-m-d',
                                            strtotime('2024-07-' . str_pad($i, 2, '0', STR_PAD_LEFT)),
                                        );
                                    @endphp
                                    <td
                                        class="border border-gray-300 px-4 py-2 
                                    @if ($date >= $agenda->start_dt_r && $date <= $agenda->end_dt_r) bg-green-500 @endif">
                                    </td>
                                @endfor
                            </tr>
                            <tr>
                                ...
                                @for ($i = 1; $i <= 30; $i++)
                                    ...
                                    <td
                                        class="border border-gray-300 px-4 py-2 
                                    @if ($date >= $agenda->start_dt_a && $date <= $agenda->end_dt_a) bg-red-500 @endif">
                                    </td>
                                @endfor
                            </tr>
                        @endforeach
